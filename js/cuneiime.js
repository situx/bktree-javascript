...
var isbktree=true;
 ...
 var loadIMEDict=function(name,bktreee){
     //alert("Loading New Dict "+name)
     if(name in imedicts){
            currentIMEDict=imedicts[name];
    }else if(bktreee && name in bktreedicts){
		currentIMEDict=bktree
		bktreedicts[name]=currentIMEDict;
	}else if(bktreee){
		        $.ajax({
    url: "ime/"+name+"_bktree_dict.json",
    dataType: 'json',
        ...
        self.keyPress = function(event){
            if (self.options.active) {
                ...
                if (pat.test(key)){ 
					var words;
					if(bktree){
						words=searchTree(currentIMEDict,self.currentText+key,2);
					}else{	
						words = currentIMEDict[self.currentText+key];
					}
